...
public class PedidoController implements Serializable {
    ...
    public void prepararVenda(){
             
        prepareCreate();//pedidos
        
        //gera parametros estaticos para a venda
        getItems();
        Pedido lastped = (items.get(items.size() - 1));//recupera o ultimo pedido
        selected.setCodigo(lastped.getCodigo() + 1); //Incrementa o codigo do pedido
        selected.setData(new Date(System.currentTimeMillis()));//seta a data
        selected.setDatahora(new Date(System.currentTimeMillis()));//seta data e hora    
